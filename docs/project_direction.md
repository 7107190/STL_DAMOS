...
is to behave as cooperative observer nodes. Scenic support actors are reduced
into semantic abnormal anchors; every semantic anchor receives one humanoid
observer and one deliverybot observer, and each observer uses the six-camera
walker layout from `/home/vvu/vv/DAMOS/sensor_config.txt`.
...
| Anchor pair policy | Implemented: one humanoid and one deliverybot per semantic anchor |
